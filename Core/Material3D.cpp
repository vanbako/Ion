        ...
        , mpCanvasCubes{}
{
	...
	while (pVSReflector->GetInputParameterDesc(mInputElementCount, &mSigParDescs[mInputElementCount]) == S_OK)
        ...
        mInputSemanticNames.resize(mInputElementCount);
        UINT svInstanceID{ 0 };
        UINT descIndex{ 0 };
        for (UINT i{ 0 }; i < mInputElementCount; ++i)
        {
                ...
                mInputSemanticNames[descIndex] = mSigParDescs[i].SemanticName;
                mpInputElementDescs[descIndex] = {
                        mInputSemanticNames[descIndex].c_str(),
